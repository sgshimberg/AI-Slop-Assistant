...
Status: Phase 3 complete (tested against real syllabi/calendars for all 6 current courses, zero left unclassified). Last updated 2026-08-13.
...
3. ✅ **Syllabus parser** — Claude API extracts weights/due dates/exam dates from real syllabi using strict JSON schemas.
   *Test: parse 2–3 actual syllabi, manually verify accuracy.* → see [docs/PHASE3.md](docs/PHASE3.md) — PASSED 2026-08-13 against all 6 real courses in `data/courses/`.
...
- 2026-08-13: **Phase 3 implemented and passed.** Added `src/integrations/syllabus.py` (`extract_text` via `pypdf`/`python-docx`, `identify_course` and `parse_course_documents` via forced Claude tool-use, `group_by_course`/`parse_all_courses`), `scripts/parse_courses.py`, and `tests/test_syllabus_parse.py` (9 tests, mocked Anthropic responses, all passing). `pypdf`/`python-docx` added to `requirements.txt`. Ran the real manual test gate against all 6 courses in `data/courses/` (ECE 109, 200, 211, 220, 301, 306) — weights, opportunistic due dates, and syllabus/calendar cross-referencing (220, 301, 306) all checked out by eye.
  - **Bug found and fixed during the manual gate**: `identify_course` had no fallback for a document that never states its course code (`ECE220-Fall25 - Course Schedule - DY Eun (2).pdf` — a bare weekly grid, confirmed by inspecting its extracted text directly) — it hallucinated a *different* wrong course code on each of two runs instead of admitting it couldn't tell, which risked silently merging a document into the wrong course's grade record. Fixed by giving `identify_course` an explicit `"UNKNOWN"` escape hatch, and having `group_by_course` route each unclassifiable document to its own `"UNKNOWN: <filename>"` key (never a shared bucket, since two different real courses' orphan documents landing there together would be its own silent-corruption bug).
  - **That limitation resolved, same day**: added a second-pass `_reconcile_unknowns` step — for each `"UNKNOWN: <filename>"` document, Claude compares its content against already-classified courses' documents (topic/chapter sequence, named tests/labs — never against another unclassified document, to avoid relocating the same corruption risk) and merges it in on a confident match, `null` otherwise. Re-ran against the real `data/courses/` dropbox: the schedule PDF now correctly reconciles into `ECE 220`, and all 6 real courses group with zero leftover `UNKNOWN` keys — full detail in docs/PHASE3.md's Known limitation section. Also fixed two smaller gaps found while re-verifying: `parse_course_documents` now defaults a missing `notes` key to `None` (the schema allows omitting it; one real response did), and warns (not errors) when a course's category weights don't sum to ~1.0, per the original Data model table's intent — neither was implemented in the first pass. Test suite grew from 9 to 14 tests covering all of this.
  - **User instruction, same day**: if the system is ever genuinely unsure which course a document belongs to, it should ask rather than guess (e.g. "I uploaded my syllabus and it doesn't give a lot of identifying information — it should ask what class this goes to"). Added an `ask_human` callback (`AskHuman` type in src/integrations/syllabus.py) as a third tier, only reached after both `identify_course` and content-based `_reconcile_unknowns` come back empty — covers the case content-matching can't: the *first* document for a brand-new course, with no sibling document yet to compare against. `scripts/parse_courses.py` wires it to a terminal prompt today (prints the filename, courses identified so far, and a text excerpt, then asks); the callback shape (`document, context -> answer`) is the same one a future WhatsApp round-trip would need, so that's a later swap, not a rewrite. Nothing in the real `data/courses/` batch triggers this tier (content-reconciliation already resolved the one case that existed), so it's verified via `tests/test_syllabus_parse.py`'s mocked scenarios only — worth a real check once uploads happen one course at a time. Test suite grew to 17.
